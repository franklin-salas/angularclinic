...
export class StaffListComponent implements OnInit {
  ...
  private getTableData(page = 1): void {
    ...
    this.staffService.listStaff(this.currentPage, this.pageSize, this.searchDataValue, this.sortBy, this.sortDirection).subscribe({
  ...
  public onDelete(data:any):void {
    ...
    sweet.fire({
      icon: "warning",
      title: "Eliminar",
      text: `Estas seguro de eliminar el empleado : ${data.name} ?`,
      showCancelButton: true,
      background: "#fff",
    }).then((result) => {
      /* Read more about isConfirmed, isDenied below */
      if (result.isConfirmed) {
        this.deleteStaff(data.id)
      } 
    });

  }

  private deleteStaff(id:number):void {
